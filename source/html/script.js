...
// Cache expiration: 30 minutes in milliseconds
const CACHE_EXPIRY = 30 * 60 * 1000;
let lastSummaryFetch = 0;
const lastFetchTimes = {}; // { camName: timestamp }

async function fetchCameras(quiet = false) {
  const loading = document.getElementById("loading");
  const refreshBtn = document.getElementById("refresh-btn");

  if (!quiet && loading) loading.style.display = "flex";
  ...
  try {
    ...
    // Only render dashboard if we are currently on it
    ...
    if (loading && !quiet) loading.style.display = "none";
...
async function manualRefresh() {
  ...
  const savedCam = currentCam;
  const savedType = currentType;
  const savedDate = currentShowingDate;
  ...
  lastSummaryFetch = 0;
  Object.keys(lastFetchTimes).forEach((cam) => (lastFetchTimes[cam] = 0));

  await fetchCameras(true);

  // Restore the view
  if (savedView === "dates") {
    prepareDates(savedCam, savedType);
  } else if (savedView === "times" || savedView === "hour") {
    // If in times or hour view, we need the full details
    try {
      const response = await fetch(`/api/cameras/${savedCam}`);
      if (!response.ok) throw new Error("Failed to fetch camera details");
      const details = await response.json();
      cameraData[savedCam] = { ...cameraData[savedCam], ...details };
      lastFetchTimes[savedCam] = Date.now();

      if (savedView === "times") {
        showTimes(savedCam, savedType, savedDate);
      } else {
        showHourTimes(savedCam, savedType, savedDate, savedHour);
      }
    } catch (error) {
...
function renderCameras(data) {
  ...
  if (cameraNames.length === 0) {
    dashboard.innerHTML =
      '<p class="subtitle" style="grid-column: 1/-1">No cameras found.</p>';
    return;
  }
...
async function prepareDates(camName, type) {
  const now = Date.now();
  const isStale =
    !lastFetchTimes[camName] || now - lastFetchTimes[camName] > CACHE_EXPIRY;

  // Check if we need to fetch full camera data (first time or stale)
  if (!cameraData[camName].photos || isStale) {
    try {
      const response = await fetch(`/api/cameras/${camName}`);
      if (!response.ok) throw new Error("Failed to fetch camera details");
      const details = await response.json();
      cameraData[camName] = { ...cameraData[camName], ...details };
      lastFetchTimes[camName] = Date.now();
    } catch (error) {
      console.error("Error:", error);
...
function nextHour() {
  ...
  if (currentIndex < currentAvailableHours.length - 1) {
    showHourTimes(
      ...
      currentType,
      currentShowingDate,
      currentAvailableHours[currentIndex + 1],
    );
  } else {
...
function viewMedia() {
  ...
  const videoUrl = relPath.toLowerCase().endsWith(".mkv")
    ? `/video/${currentCam}/${relPath}`
    : fileUrl;
  ...
  if (currentTimeIndex < currentTimestamps.length - 1) {
    ...
    const nextUrl = nextPath.toLowerCase().endsWith(".mkv")
      ? `/video/${currentCam}/${nextPath}`
      : `/data/${currentCam}/${nextPath}`;
    if (currentType === "photos") {
...
async function switchCamera(newCamName) {
  ...
  const now = Date.now();
  const isStale =
    !lastFetchTimes[newCamName] ||
    now - lastFetchTimes[newCamName] > CACHE_EXPIRY;

  // Ensure we have data for the new camera (first time or stale)
  if (!cameraData[newCamName].photos || isStale) {
    try {
      const response = await fetch(`/api/cameras/${newCamName}`);
      if (!response.ok) throw new Error("Failed to fetch camera details");
      const details = await response.json();
      cameraData[newCamName] = { ...cameraData[newCamName], ...details };
      lastFetchTimes[newCamName] = Date.now();
    } catch (error) {
      console.error("Error:", error);
...
function goHome() {
  ...
  document.getElementById("sub-title").textContent =
    "Real-time status of your security system";
}
